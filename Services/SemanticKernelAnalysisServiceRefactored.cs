...
/// <summary>
/// Refactored analysis service with better separation of concerns
/// </summary>
public sealed class SemanticKernelAnalysisServiceRefactored : IAnalysisService
{
    ...
    private readonly ILogger<SemanticKernelAnalysisServiceRefactored> _logger;
    private readonly string _modelName;

    private static readonly ActivitySource ActivitySource = new("PullRequestAnalyzer", "1.0.0");

    public SemanticKernelAnalysisServiceRefactored(
        IPromptService promptService,
        IJsonParsingService jsonParser,
        IValidationService validationService,
        IConfiguration configuration,
        ILogger<SemanticKernelAnalysisServiceRefactored> logger)
    {
    ...
    private void RecordLlmMetrics(Activity? span, Activity? parentActivity, string? response, long latencyMs)
    {
        ...
        var estimatedTokens = response.Length / _config.Llm.PromptTokensEstimate;
        var cost = EstimateCost(estimatedTokens);

        span?.SetTag("gen_ai.usage.completion_tokens", estimatedTokens);
        span?.SetTag("gen_ai.response.finish_reasons", new[] { "stop" });
        span?.SetStatus(ActivityStatusCode.Ok);

        parentActivity?.SetTag("llm.output_tokens", estimatedTokens);
