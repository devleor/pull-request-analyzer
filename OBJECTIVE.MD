...
1. **We must have access to your full source code.**
    
    The final submission must be a GitHub repository (public or private with access granted to us).
    
2. **Do not worry about deployment, infrastructure, cloud setup, or production hosting.**
    
    This should run locally. No need to deploy anywhere.
    
3. **You are expected to use AI tools.**
    
    We encourage using tools such as Claude Code, Cursor, Copilot, etc.
    
4. **You must attach your coding session(s).**
    
    As part of the submission, include:
    
    - Exported coding sessions (if your tool supports export)
    - Or transcripts/logs
    - Or screen recordings (if export is not available)
    
    The goal is for us to understand:
    
    - How you structured prompts
    - How you iterated
    - How you corrected or refined outputs
    - How you used AI to reach the final solution
5. **We may schedule a follow-up coding session.**
    
    In that session, we may:
    
    - Ask you to walk through your architecture
    - Extend or modify part of the solution live
    - Discuss how you used AI tools to build it

We are evaluating your engineering thinking and your ability to leverage modern tooling effectively.
...
- Full source code
- All necessary files to run the project locally
- Example PR JSON input
- Clear project structure

We must have access to this repository.
...
- Setup instructions
- Dependencies
- Environment variables required (e.g. GITHUB_TOKEN, OPENAI_API_KEY)
- How to run the API server
- How to generate PR JSON
- How to trigger the analysis
- Example request/response

The goal is that we can clone the repo and run it without guessing.

---

### **3) Coding Session Artifacts**

Attach your AI coding session artifacts, including:
...
- How your LLM workflow is structured
- How you handle large diffs (chunking, filtering, summarization, etc.)
- Tradeoffs you made
- Known limitations
...
1. Engineering clarity and structure
2. Reproducibility (can we run it easily?)
3. Quality of the analysis (grounded in diffs, not hallucinated)
4. Workflow design (prompting, chunking, schema consistency)
5. Judgment and tradeoffs
6. Your ability to explain and extend the system during a live follow-up session